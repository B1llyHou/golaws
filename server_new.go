...
// startServer initializes the server, registers the Engine, and listens on port 8080.
func startServer() {
	engine := new(Engine)
	if err := rpc.Register(engine); err != nil {
		log.Fatalf("failed to register RPC engine: %v", err)
	}
	...
	// Run connection handler in a separate goroutine
	go func() {
		for {
